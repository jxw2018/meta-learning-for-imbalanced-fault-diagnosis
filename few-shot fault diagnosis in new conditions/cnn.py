...
from pre_train_cnn import pre_train
...
def main():
    ...
    g_mean = np.zeros(12)
    for i in range(10):
        model = pre_train(args, config_pre)
    
        train_acc[i], test_acc[i], test_confusion, f_score[i], g_mean[i] = model(x_train, y_train, x_test, y_test)
    ...
    sheet = book.add_sheet('cnn',cell_overwrite_ok=True)   
    for i in range(4):
        for j in range(4):
    ...
    for i in range(12):
        ...
        sheet.write(i, 7, f_score[i])
        sheet.write(i, 8, g_mean[i])
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--pre_epoch', type=int, help='pre train epoch number', default=4)
