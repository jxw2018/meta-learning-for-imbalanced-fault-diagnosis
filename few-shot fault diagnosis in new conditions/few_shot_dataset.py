...
#        mode, batchsz, n_way, k_shot, k_query))
        ...
        # 归一化
#        scaler = MinMaxScaler()
#        self.data['data'] = scaler.fit_transform(self.data['data'])
...
#            random.shuffle(query_x)
            support_x = np.array(support_x).reshape(self.setsz, self.resize)
            support_y = np.array(support_y).reshape(self.setsz)
            query_x = np.array(query_x).reshape(self.querysz, self.resize)
            query_y = np.array(query_y).reshape(self.querysz)
            ...
            np.random.seed(111)
            np.random.shuffle(query_y)

            
            support_x = support_x.tolist()
...
#            query_y_relative[query_y == l] = idx
        ...
        return support_x.unsqueeze(1), support_y, query_x.unsqueeze(1), query_y
